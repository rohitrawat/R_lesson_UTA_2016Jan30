...
We can paste all of these commands into a single file, maybe for ease in recall, for repetition, or quickly doing something again. R scripts have the extension `.R`.
...
Syntax high
indentation
var name completion (like bash Tab)
bracket completion
...
Caveat - script runs but may fail


## Exercise

Write and run your own script
Set x to 25
Set y to 15
Calculate the sum and store it in z
print z
...
paste('centered min, mean, and max are:', min(centered), ',', mean(centered), ',', max(centered))
```	
